...
public abstract class EntitySqlRepository<T> : IRepository<T>
    where T : class, IEntityModel
{
    private readonly ServerDbContext _dbContext;

    public EntitySqlRepository(ServerDbContext dbContext)
    {
    ...
    {
        var entities = await LoadEntity(_dbContext.Set<T>())
            .WhereActive()
            .ToListAsync();
    ...
    {
        var entity = await LoadEntity(_dbContext.Set<T>())
            .WhereActive()
            .WhereId(id)
            .FirstOrDefaultAsync();
    ...
    public async Task<ServiceResult<T>> Update(T entity)
    {
        var dbEntity = await LoadEntity(_dbContext.Set<T>())
            .WhereActive()
            .WhereId(entity.Id)
            .FirstOrDefaultAsync();
        ...
        if (dbEntity == null)
        {
            var error = new ServiceError($"{typeof(T).Name} not found", $"No {typeof(T).Name} could be located for id: {entity.Id}", 404);
            return new ServiceResult<T>(error);
        }

        UpdateFields(dbEntity: dbEntity, entity: entity);

        return new ServiceResult<T>(dbEntity);
    }

    protected abstract void UpdateFields(T dbEntity, T entity);

    protected abstract IQueryable<T> LoadEntity(DbSet<T> dbSet);
}
